  ...
  public static void almostSorted(List<Integer> arr) {
    ...
    if (mismatched.size() > 2) {
      ...
      if (!isSorted) {
        ...
        List<Integer> reversingSubList =
            arr.subList(mismatched.get(0), mismatched.get(mismatched.size() - 1) + 1);
        ...
        if (isSorted(reversingSubList.stream().mapToInt(Integer::intValue).toArray())
          ...
          System.out.println(
              "reverse "
                  + (mismatched.get(0) + 1)
                  + " "
                  + (mismatched.get(mismatched.size() - 1) + 1));
      ...
      System.out.println("yes");
      System.out.println(
          "swap " + (mismatched.get(0) + 1) + " " + (mismatched.get(mismatched.size() - 1) + 1));
    }
  }
