...
public class LuckBalance {
  ...
  public static int luckBalance(int k, List<List<Integer>> contests) {
    int luckBalance = 0;
    int importantContestsCount =
        contests.stream()
            .filter(contest -> contest.get(1) == 1)
    ...
    if (k >= importantContestsCount) {
      ...
      contests.sort(
          Comparator.comparingInt((List<Integer> contest) -> contest.get(1))
              .reversed()
              .thenComparingInt(contest -> contest.get(0)));
      luckBalance =
          luckBalance
              + contests.subList(mustwin, contests.size()).stream()
                  .mapToInt(contest -> contest.get(0))
                  .sum();
      luckBalance =
          luckBalance
              - contests.subList(0, mustwin).stream().mapToInt(contest -> contest.get(0)).sum();
    }
